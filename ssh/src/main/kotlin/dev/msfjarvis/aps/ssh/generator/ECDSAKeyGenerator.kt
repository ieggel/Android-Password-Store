...
class ECDSAKeyGenerator(private val isStrongBoxSupported: Boolean) : SSHKeyGenerator {
  ...
  override fun generateKey(requiresAuthentication: Boolean): KeyPair {
    ...
    val parameterSpec = KeyGenParameterSpec.Builder(KEYSTORE_ALIAS, KeyProperties.PURPOSE_SIGN).run {
      setKeySize(256)
    ...
    val keyPair = KeyPairGenerator.getInstance(algorithm, PROVIDER_ANDROID_KEY_STORE).run {
